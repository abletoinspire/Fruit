# python train_C.py --train-img-dir ./edited_dataset/train/images --train-ann ./edited_dataset/train/annotations.json --val-img-dir ./edited_dataset/val/images --val-ann ./edited_dataset/val/annotations.json --out-dir ./runs_C_condqual --cache-dir ./mask_cache_C --precache --img-size 512 --epochs 10
import os
import json
import math
import time
...
class Config:
    ...
    quality_weight_scope: str = "per_type"
...
def compute_obj_counts(idx: CocoIndex) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:


    t_counts = np.zeros((idx.num_types,), dtype=np.int64)
    q_counts_global = np.zeros((idx.num_qualities,), dtype=np.int64)
    q_counts_by_type = np.zeros((idx.num_types, idx.num_qualities), dtype=np.int64)
    ...
    for a in idx.annotations:
        ...
        t_counts[t_id] += 1
        q_counts_global[q_id] += 1
        q_counts_by_type[t_id, q_id] += 1

    return t_counts, q_counts_global, q_counts_by_type
...
class UNetConditionalQuality(nn.Module):
    ...
    def __init__(self, base_c: int, num_types: int = 3, num_qualities: int = 2, head_drop: float = 0.10):
        super().__init__()
        assert num_types == 3, "This script assumes 3 fruit types."
        assert num_qualities == 2, "This script assumes 2 quality classes."
    ...
    def forward(self, x):
        ...
        qual_logits = []
        for head in self.head_quality_experts:
            qual_logits.append(head(x))
        qual_logits = torch.stack(qual_logits, dim=1)
...
def object_ce_loss_type_from_gt_components(
    type_logits_fp32: torch.Tensor,
    ...
    device = type_logits_fp32.device
    B = type_logits_fp32.size(0)

    obj_logits = []
    obj_targets = []

    for b in range(B):
        tmap = type_gt[b].detach().cpu().numpy()
        for t in range(num_types):
            fg = (tmap == t).astype(np.uint8)
            if fg.sum() == 0:
                continue
            ...
            for lbl in range(1, nlab):
                ...
                if area < int(min_area):
                    continue
                m = (labels == lbl)
                m_t = torch.from_numpy(m).to(device=device, dtype=torch.bool)
                l = type_logits_fp32[b, :, :, :][:, m_t].mean(dim=1)
                obj_logits.append(l)
                obj_targets.append(int(t))

    if len(obj_logits) == 0:
        return type_logits_fp32.sum() * 0.0
...
def object_ce_loss_quality_conditional(
    qual_logits_fp32: torch.Tensor,
    type_gt: torch.Tensor,
    ...
    num_qualities: int,
    min_area: int,
    qual_w_by_type: Optional[List[Optional[torch.Tensor]]] = None,
    label_smoothing: float = 0.0,
) -> torch.Tensor:


    device = qual_logits_fp32.device
    B = qual_logits_fp32.size(0)

    logits_by_t: List[List[torch.Tensor]] = [[] for _ in range(num_types)]
    targets_by_t: List[List[int]] = [[] for _ in range(num_types)]
    total_obj = 0

    for b in range(B):
        tmap = type_gt[b].detach().cpu().numpy()
        qmap = qual_gt[b].detach().cpu().numpy()

        for t in range(num_types):
            fg = ((tmap == t) & (qmap != int(ignore_index))).astype(np.uint8)
            if fg.sum() == 0:
                continue
            nlab, labels, stats, _ = cv2.connectedComponentsWithStats(fg, connectivity=8)
            if nlab <= 1:
            ...
            for lbl in range(1, nlab):
                ...
                if area < int(min_area):
                    continue
                m = (labels == lbl)
                gt_vals = qmap[m].astype(np.int64)
                if gt_vals.size == 0:
                    continue
                gt_q = int(np.bincount(gt_vals, minlength=num_qualities).argmax())

                m_t = torch.from_numpy(m).to(device=device, dtype=torch.bool)
                l = qual_logits_fp32[b, t, :, :, :][:, m_t].mean(dim=1)
                logits_by_t[t].append(l)
                targets_by_t[t].append(gt_q)
                total_obj += 1

    if total_obj == 0:
        return qual_logits_fp32.sum() * 0.0

    loss_sum = 0.0
    for t in range(num_types):
        if len(logits_by_t[t]) == 0:
            continue
        X = torch.stack(logits_by_t[t], dim=0)
        y = torch.tensor(targets_by_t[t], device=device, dtype=torch.long)
        w = None
        if qual_w_by_type is not None:
            w = qual_w_by_type[t]
        try:
            loss_t = F.cross_entropy(X, y, weight=w, label_smoothing=float(label_smoothing))
        except TypeError:
            loss_t = F.cross_entropy(X, y, weight=w)

        loss_sum += loss_t * (float(len(logits_by_t[t])) / float(total_obj))
...
def select_quality_logits_by_type(
    ...
    B, T, Q, H, W = qual_logits.shape
    out = torch.zeros((B, Q, H, W), device=qual_logits.device, dtype=qual_logits.dtype)
    for t in range(num_types):
        m = (type_map == t).unsqueeze(1)
        out = torch.where(m, qual_logits[:, t, :, :, :], out)
    return out
...
def save_vis_batch(
    ...
    type_pr = torch.argmax(type_logits[:n], dim=1)


    qual_sel = select_quality_logits_by_type(qual_logits[:n], type_pr, num_types=3)
    qual_pr = torch.argmax(qual_sel, dim=1).detach().cpu().numpy().astype(np.uint8)

    type_pr_np = type_pr.detach().cpu().numpy().astype(np.uint8)
    ...
    for i in range(n):
        ...
        tcol = pal_type[type_pr_np[i].reshape(-1)].reshape(h, w, 3)
...
def run_epoch(
    ...
    qual_w_by_type: Optional[List[Optional[torch.Tensor]]],
    qual_w_global: Optional[torch.Tensor],
    ...
    q_acc_oracle_sum = 0.0
    q_miou_oracle_sum = 0.0
    q_acc_predtype_sum = 0.0
    q_miou_predtype_sum = 0.0
    ...
    with ctx:
        ...
        for imgs, fruit_gt, type_gt, qual_gt, _names in pbar:
            ...
            loss_type_obj = object_ce_loss_type_from_gt_components(
                type_logits_fp32=type_logits_f,
                type_gt=type_gt,
            ...
            loss_qual_px = qual_logits_f.sum() * 0.0
            if cfg.w_qual_px > 0:
                total_pix = 0
                loss_pix_sum = 0.0
                for t in range(num_types):
                    m = (type_gt == t) & (qual_gt != cfg.ignore_index)
                    if not m.any():
                        continue
                    wq = None
                    if cfg.quality_weight_scope == "per_type" and qual_w_by_type is not None:
                        wq = qual_w_by_type[t]
                    elif cfg.quality_weight_scope == "global":
                        wq = qual_w_global

                    l = masked_pixel_ce_loss(
                        logits=qual_logits_f[:, t, :, :, :],
                        target=qual_gt,
                        mask=m,
                        weight=wq,
                        label_smoothing=cfg.label_smoothing,
                    ...
                    pix = int(m.sum().item())
                    total_pix += pix
                    loss_pix_sum = loss_pix_sum + l * (pix / max(1.0, float(total_pix)))

                loss_qual_px = loss_pix_sum


            loss_qual_obj = object_ce_loss_quality_conditional(
                qual_logits_fp32=qual_logits_f,
                type_gt=type_gt,
                qual_gt=qual_gt,
                ignore_index=cfg.ignore_index,
                num_types=num_types,
                ...
                min_area=cfg.obj_loss_min_area,
                qual_w_by_type=(qual_w_by_type if cfg.quality_weight_scope == "per_type" else None),
                label_smoothing=cfg.label_smoothing,
            ...
            qual_sel_oracle = select_quality_logits_by_type(qual_logits, type_gt, num_types=num_types)
            q_acc_oracle = masked_acc(qual_sel_oracle, qual_gt, cfg.ignore_index)
            q_miou_oracle = masked_miou(qual_sel_oracle, qual_gt, num_qualities, cfg.ignore_index)


            qual_sel_pred = select_quality_logits_by_type(qual_logits, type_pred, num_types=num_types)
            q_acc_pred = masked_acc(qual_sel_pred, qual_gt, cfg.ignore_index)
            q_miou_pred = masked_miou(qual_sel_pred, qual_gt, num_qualities, cfg.ignore_index)

            iou_sum += iou
            dice_sum += dice
            type_acc_sum += type_acc
            type_miou_sum += type_miou
            q_acc_oracle_sum += q_acc_oracle
            q_miou_oracle_sum += q_miou_oracle
            q_acc_predtype_sum += q_acc_pred
            q_miou_predtype_sum += q_miou_pred
            n_batches += 1


            if (not train) and (cm_pack is not None):
                type_pr = type_pred
                qual_pr_oracle = torch.argmax(qual_sel_oracle, dim=1)
                qual_pr_pred = torch.argmax(qual_sel_pred, dim=1)
                for b in range(bs):
                    update_pixel_cm(cm_pack["cm_type_px"], type_gt[b], type_pr[b], cfg.ignore_index, num_types)
                    update_pixel_cm(cm_pack["cm_qual_px_oracle"], qual_gt[b], qual_pr_oracle[b], cfg.ignore_index, num_qualities)
                    update_pixel_cm(cm_pack["cm_qual_px_predtype"], qual_gt[b], qual_pr_pred[b], cfg.ignore_index, num_qualities)

            pbar.set_postfix(
                loss=float(loss.item()),
                fruit_iou=iou,
                type_acc=type_acc,
                q_oracle=q_acc_oracle,
                q_pred=q_acc_pred,
            )
    ...
    return {
        ...
        "quality_oracle_acc": q_acc_oracle_sum / max(1, n_batches),
        "quality_oracle_miou": q_miou_oracle_sum / max(1, n_batches),
        "quality_predtype_acc": q_acc_predtype_sum / max(1, n_batches),
        "quality_predtype_miou": q_miou_predtype_sum / max(1, n_batches),
    }
...
def main():
    ...
    ap.add_argument("--quality-weight-scope", type=str, default="per_type", choices=["per_type", "global", "none"])
    ...
    cfg = Config(
        ...
        quality_weight_scope=args.quality_weight_scope,
    ...
    idx = train_ds.idx
    num_types = idx.num_types
    num_qualities = idx.num_qualities
    if num_types != 3 or num_qualities != 2:
        raise ValueError(f"Expected 3 types and 2 qualities, got types={num_types}, qualities={num_qualities}")

    if len(idx.catid_to_type_quality) == 0:
        raise RuntimeError(
            "No COCO categories mapped to (type,quality). Check category names: "
            "expected 'fresh_apple'/'rotten_banana'/etc."
        )
    ...
    t_counts, q_counts_global, q_counts_by_type = compute_obj_counts(idx)
    ...
    qual_w_global_t = None
    qual_w_by_type_t: List[Optional[torch.Tensor]] = [None, None, None]

    if cfg.use_class_weights and cfg.weight_mode != "none" and cfg.quality_weight_scope != "none":
        if cfg.quality_weight_scope == "global":
            qw = make_class_weights(q_counts_global, cfg.weight_mode)
            qual_w_global_t = torch.tensor(qw, device=cfg.device, dtype=torch.float32)
        else:
            for t in range(num_types):
                qw = make_class_weights(q_counts_by_type[t], cfg.weight_mode)
                qual_w_by_type_t[t] = torch.tensor(qw, device=cfg.device, dtype=torch.float32)

    model = UNetConditionalQuality(cfg.base_c, num_types=num_types, num_qualities=num_qualities, head_drop=0.10).to(cfg.device)
    ...
    print(f"Types: {idx.type_names} | Qualities: {idx.quality_names}")
    print(f"Obj counts type: {t_counts.tolist()}")
    print(f"Obj counts quality global: {q_counts_global.tolist()} | by_type:\n{q_counts_by_type}")
    if type_w_t is not None:
        print(f"Type weights: {type_w_t.detach().cpu().numpy().round(3).tolist()}")
    if cfg.quality_weight_scope == 'per_type':
        for t in range(num_types):
            if qual_w_by_type_t[t] is not None:
                print(f"Qual weights [{idx.type_names[t]}]: {qual_w_by_type_t[t].detach().cpu().numpy().round(3).tolist()}")
    elif cfg.quality_weight_scope == 'global' and qual_w_global_t is not None:
        print(f"Qual weights [global]: {qual_w_global_t.detach().cpu().numpy().round(3).tolist()}")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        tr = run_epoch(
            ...
            train=True,
            type_w=type_w_t,
            qual_w_by_type=qual_w_by_type_t,
            qual_w_global=qual_w_global_t,
            cm_pack=None,
        )
        ...
        if do_cm:
            cm_pack = {
                ...
                "cm_qual_px_oracle": np.zeros((num_qualities, num_qualities), dtype=np.int64),
                "cm_qual_px_predtype": np.zeros((num_qualities, num_qualities), dtype=np.int64),
            }
        ...
        score = (0.40 * va["fruit_iou"]) + (0.30 * va["type_acc"]) + (0.30 * va["quality_predtype_acc"])
        ...
        body = (
            f"  train: loss={tr['loss']:.4f} fruit_iou={tr['fruit_iou']:.4f} type_acc={tr['type_acc']:.4f} "
            f"q_oracle={tr['quality_oracle_acc']:.4f} q_pred={tr['quality_predtype_acc']:.4f}\n"
            f"  val:   loss={va['loss']:.4f} fruit_iou={va['fruit_iou']:.4f} fruit_dice={va['fruit_dice']:.4f} "
            f"type_acc={va['type_acc']:.4f} type_mIoU={va['type_miou']:.4f} "
            f"q_oracle={va['quality_oracle_acc']:.4f} q_pred={va['quality_predtype_acc']:.4f}"
        )
        print(header)
        print(body)
        ...
        if do_cm and cm_pack is not None:
            ...
            save_cm_png(cm_pack["cm_type_px"], idx.type_names, f"Type CM (pixel) [val] e{epoch:03d}", os.path.join(out_cms, f"e{epoch:03d}_type_px.png"))
            save_cm_png(cm_pack["cm_qual_px_oracle"], idx.quality_names, f"Quality CM ORACLE (pixel) [val] e{epoch:03d}", os.path.join(out_cms, f"e{epoch:03d}_qual_px_oracle.png"))
            save_cm_png(cm_pack["cm_qual_px_predtype"], idx.quality_names, f"Quality CM PREDTYPE (pixel) [val] e{epoch:03d}", os.path.join(out_cms, f"e{epoch:03d}_qual_px_predtype.png"))
            ...
            torch.save(
                {
                    ...
                    "cfg": cfg.__dict__,
                    "type_names": idx.type_names,
                    "quality_names": idx.quality_names,
                    "variant": "C_conditional_quality_experts",
                },
                ckpt_path,
            )
